...
//            paymentPlatform = Constant.DB_ORDER_PAYMENT_TYPE_WEIXINPAY;//通过浏览器临时测试时使用
      ...
      if (!Strings.isNullOrEmpty(selectProdId)) {
        for (PromotionInfoResponse promotionInfoResponse : this.prods) {
          for (PromotionCategoryResult promotionCategoryResult : promotionInfoResponse
              .getProductCategoryList()) {
            for (PromotionInfoWS promotionInfoWS : promotionCategoryResult
  ...
  public String toOrderPackage() {
    ...
    if (validateOrderEntryInput()) {
      ...
      if (!Strings.isNullOrEmpty(accountNum)) {
        ...
        List<VehicleInfoResponse> tmpVehicleList = client
            .getVehicleInfo(accountInfo.getVin(), "", "");
        ...
        if ((vin != null && vin.indexOf("*") > -1) || (mobilePhone != null
        ...
        FacesUtils.setManagedBeanInSession(Constant.ACCOUNT_NUM,
...
//根据账户绑定openid
      ...
      boolean flag = PaymentServiceClient.getInstance()
          .createWechatUserProfile(accountNum, "", openId, userInfo,
              accountInfo.getVin());//保存绑定信息accountInfo.getvin
      if (flag) {
...
//          List<Coupon> getCoupons = client
//              .findCouponsByPromotionId(accountNum, promotionCoupon.getId());
...
//                logger.info("promotionCoup数量========"+promotionCoup.size());
  ...
  private boolean checkOneCentPromotionTaken(VehicleInfoResponse vehicle) {
    ...
    if (Constant.WECHAT_STATE_PROMOTION.equalsIgnoreCase(state)) {
      PurchaseProductResponse response = client
          .checkPurchaseProduct(vehicle.getVin(), Constant.DB_PACKAGE_TYPE_WIFI_PROMOTION);
      if (Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS
          .equalsIgnoreCase(response.getRespCode())) {
        FacesContext context = FacesContext.getCurrentInstance();
        context.addMessage(null, new FacesMessage(
    ...
    if (validateVehicleSelectionInput()) {
      ...
      if (accountType.equalsIgnoreCase(Constant.DB_ACCOUNT_TYPE_SUBSCRIBER) &&
        ...
        context.addMessage(null, new FacesMessage(
            ...
            MessageBundle.getValue("web.b2c.order.error.account.status"), ""));
      }
      ...
      boolean flag = PaymentServiceClient.getInstance()
          .createWechatUserProfile(accountNum, "", openId, userInfo, accountInfo.getVin());//保存绑定信息
      if (flag) {
        logger.info("createRenewalServiceUserProfileFlag == true");
      }
      return ViewPage.LINK2OrderPackage0;
...
//        oId ="o8rKvs2EeP8RdQSk0itqWSQj90hc";
    ...
    logger.info("accountNum ==>" + ESAPI.encoder()
        .encodeForHTML(String.valueOf(selectedVehicle.getAcctNum())));
...
//        paymentPlatform="111005";
    ...
    if (!paymentOrderResponse.getRespCode()
        .equals(Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS)) {
      ...
      orderPackagePop = "创建订单失败，原因：" + paymentOrderResponse.getRespMsg()
          + "请重试或致电<span style=\"text-decoration: underline;\" class=\"span2\">400-898-0050</span>联系梅赛德斯-奔驰智能互联服务中心寻求帮助";
      return ViewPage.LINK2OrderPackage0;
    }
    //根据账户绑定openid
    //openId="o8rKvs6BZi-LQzX_UdNL0X0TL6T4";
...
//            primePrice=Double.parseDouble(selectProd.getPromotionDesc5A());
    ...
    transactionRequest
        .setRemark("微信支付，OpenID:" + FacesUtils.getManagedBeanInSession(Constant.OPEN_ID));
    transactionRequest.setRespcode("00A4");
    transactionRequest.setResponse("已下单，等待支付");
    transactionRequest.setSeraialno(serialNo);
    logger.debug("creating transaction");
    TransactionResponse transactionResponse = client
        .createTransaction(paymentOrderResponse.getOrderNum(), transactionRequest);
    ;
    if (!transactionResponse.getRespcode()
        .equals(Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS)) {
      logger.error("create new transaction failed, error message: " + ESAPI.encoder()
          .encodeForHTML(transactionResponse.getRespMsg()));
            /*orderAccountInfoPop = "创建交易失败，原因：" + transactionResponse.getRespMsg();*/
      context.addMessage(null, new FacesMessage(
          FacesMessage.SEVERITY_ERROR, "创建交易失败，原因：" + transactionResponse.getRespMsg()
          + "<br>请重试或致电<span class='span2'>400-898-0050</span>联系<br>梅赛德斯-奔驰智能互联<br>服务中心寻求帮助", ""));
            /*return ViewPage.LINK2OrderAccountInfo;*/
      orderPackagePop = "创建订单失败，原因：" + transactionResponse.getRespMsg()
          + "请重试或致电<span style=\"text-decoration: underline;\" class=\"span2\">400-898-0050</span>联系梅赛德斯-奔驰智能互联服务中心寻求帮助";
      return ViewPage.LINK2OrderPackage0;
...
//            FacesUtils.setManagedBeanInSession(Constant.OPEN_ID, oId);
      ...
      logger.debug("open_id=" + ESAPI.encoder()
          .encodeForHTML(FacesUtils.getManagedBeanInSession(Constant.OPEN_ID).toString()));
      ...
      logger.debug("orderId=" + ESAPI.encoder()
          .encodeForHTML(transactionNo + paymentOrderResponse.getOrderNum()));
    ...
    System.out.println(
        "accountNum==" + accountNum + "isUsed==" + 0 + "currentDate==" + currentDate + "sProdId=="
            + sProdId);
//        String pakgTag="0";
...
//                transactionNo = getTransactionNo();
        ...
        if (parentResult && serviceResult && transActionResult) {
          for (String s : couponArray) {
            boolean couponHistoryResult = client
                .updateCouponHistory(orderNumber, amount + "", s, accountNum);
  ...
  public String toOneCentPromotion() {
    ...
    if (productInfo.getBaseProductList().size() > 0) {
      ...
      logger.info("selectedBaseProduct=" + ESAPI.encoder()
          .encodeForHTML(selectedBaseProduct.getProdId() + ""));
    }
    return ViewPage.LINK2OrderEntry;
  }
  ...
  private boolean validateOrderEntryInput() {
    ...
    if (Strings.isNullOrEmpty(accountNum) && Strings.isNullOrEmpty(mobilePhone) && Strings
        .isNullOrEmpty(vin)) {
...
//    }
  ...
  public ProductInfo getSelectedBaseProduct() {
    ...
    if (Constant.WECHAT_STATE_PROMOTION.equalsIgnoreCase(state)) {
      productInfo = client.getPromotionProdInfo();
      if (selectedBaseProduct == null && productInfo != null
          && productInfo.getBaseProductList().size() > 0) {
        selectedBaseProduct = productInfo.getBaseProductList().get(0);
      }
    }
  ...
  public String toAlipay() throws Exception {
    try {
      ...
      String out_trade_no = new String(
          FacesUtils.getRequestParameter("WIDout_trade_no").getBytes("ISO-8859-1"), "UTF-8");
      ...
      String subject = new String(
          FacesUtils.getRequestParameter("WIDsubject").getBytes("ISO-8859-1"), "UTF-8");
      //必填

      //付款金额
      //        String total_fee = new String(request.getParameter("WIDtotal_fee").getBytes("ISO-8859-1"),"UTF-8");
      String total_fee = new String(
          FacesUtils.getRequestParameter("WIDtotal_fee").getBytes("ISO-8859-1"), "UTF-8");

      //必填

      //请求业务参数详细
      String req_dataToken =
          "<direct_trade_create_req><notify_url>" + notify_url + "</notify_url><call_back_url>"
              + call_back_url + "</call_back_url><seller_account_name>" + AlipayConfig.seller_email
...
//        return ViewPage.LINK2OrderAlipayRequest;
  ...
  public String toOrderUpgradeEntry(PackageInfoResponse currentPkg, AccountInfoResponse accountInfo,
      String openId, String fromFlag) {
...
//            paymentPlatform = Constant.DB_ORDER_PAYMENT_TYPE_WEIXINPAY;//通过浏览器临时测试时使用
      ...
      upgradeResponse = PaymentServiceClient.getInstance()
          .getNewPackageAfterUpgrade(upgradeRequest);
      if (!Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS
          .equals(upgradeResponse.getRespCode())) {
  ...
  private boolean validateToBeUpgratedPkgSelection() {
    ...
    FacesContext context = FacesContext.getCurrentInstance();
    if (accountInfo == null || Strings.isNullOrEmpty(accountInfo.getAccountNum()) && Strings
        .isNullOrEmpty(accountInfo.getVin())) {
  ...
  public String toOrderUpgradePayment() {
    ...
    String oId = (String) FacesUtils.getManagedBeanInSession(Constant.OPEN_ID);
    logger.info("upgrade....original PkgId  == " + ESAPI.encoder()
        .encodeForHTML(upgradeResponse.getOriginalPackageId()));
    logger.info("upgrade....new PkgId  == " + ESAPI.encoder()
        .encodeForHTML(upgradeResponse.getNewPackageId()));
    logger.info("upgrade...gap price ==" + ESAPI.encoder()
        .encodeForHTML(String.valueOf(upgradeResponse.getUpgradeGapPrice())));
    logger.info("paymentPlatform ==>" + ESAPI.encoder().encodeForHTML(paymentPlatform));
    logger.info("oId ==>" + ESAPI.encoder().encodeForHTML(oId));
    ...
    if (!upgradeResponse.getRespCode().equals(Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS)) {
      ...
      context.addMessage(null, new FacesMessage(
          FacesMessage.SEVERITY_ERROR, "创建订单失败，原因：" + upgradeResponse.getRespMsg(), ""));
            /*return ViewPage.LINK2OrderAccountInfo;*/
      orderUpgradePop = "创建订单失败，原因：" + upgradeResponse.getRespMsg()
          + "请重试或致电<span style=\"text-decoration: underline;\" class=\"span2\">400-898-0050</span>联系梅赛德斯-奔驰智能互联服务中心寻求帮助";
      return ViewPage.LINK2OrderPackageUpgrade;
    }
...
//        String serialNo = getSerialNo();
    ...
    logger.debug("creating transaction");
    TransactionResponse transactionResponse = client
        .createTransaction(upgradeResponse.getUpgratedParentOrderNum(), transactionRequest);
    ;
    if (!transactionResponse.getRespcode()
        .equals(Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS)) {
      logger.error("create new transaction failed, error message: " + ESAPI.encoder()
      ...
      context.addMessage(null, new FacesMessage(
          ...
          + "<br>请重试或致电<span class='span2'>400-898-0050</span>联系<br>梅赛德斯-奔驰智能互联<br>服务中心寻求帮助", ""));
            /*return ViewPage.LINK2OrderAccountInfo;*/
      orderUpgradePop = "创建订单失败，原因：" + transactionResponse.getRespMsg()
          + "请重试或致电<span style=\"text-decoration: underline;\" class=\"span2\">400-898-0050</span>联系梅赛德斯-奔驰智能互联服务中心寻求帮助";
      return ViewPage.LINK2OrderPackageUpgrade;
...
//                tpWxPay.setOrderId(paymentOrderResponse.getOrderNum());
      tpWxPay.setOrderId(
          (int) (Math.random() * 10) + transactionNo + upgradeResponse.getUpgratedParentOrderNum());
      logger.debug("orderId=" + ESAPI.encoder()
          .encodeForHTML(transactionNo + upgradeResponse.getUpgratedParentOrderNum()));
      tpWxPay.setSpbillCreateIp("127.0.0.1");
      tpWxPay.setTotalFee(String.valueOf(amount));
      logger.debug("totalFee=" + ESAPI.encoder().encodeForHTML(String.valueOf(amount)));
      wechatPrepayResponse = demo.getPackage(tpWxPay);
...
//        List<Transaction> list = client.checkTransactionPaied(transactionNos.get(0));
    queryOrderByParentOrderNumResponse = client
        .queryOrderByParentOrderNum(parentOrderNum, accountNum);
    logger.info("queryOrderByParentOrderNumResponse================>" + transactionType);
    List<QueryChildOrdersByParentOrderNumResponse> orders = client
        .queryChildOrdersByParentOrderNum(parentOrderNum);
    if (orders == null || orders.size() == 0) {
      //没创建订单
      transactionType = "1";
    } else {
      if (!queryOrderByParentOrderNumResponse.getOrderStatus().equals(Constant.DB_ORDER_STATUS_NEW)
          && !queryOrderByParentOrderNumResponse.getOrderStatus()
          .equals(Constant.DB_ORDER_STATUS_PENDING_PAYMENT)
          && !queryOrderByParentOrderNumResponse.getOrderStatus()
          .equals(Constant.DB_ORDER_STATUS_PAYMENT_FAILED)
          && !queryOrderByParentOrderNumResponse.getOrderStatus()
          .equals(Constant.DB_ORDER_STATUS_PAYMENT_CANCELED)) {
        //订单已支付
        transactionType = "2";
      }
      if (queryOrderByParentOrderNumResponse.getOrderStatus()
          .equals(Constant.DB_ORDER_STATUS_PAYMENT_CANCELED)) {
        //订单失效关闭
        transactionType = "3";
      }
      ...
      if (queryOrderByParentOrderNumResponse.getOrderStatus()
          .equals(Constant.DB_ORDER_STATUS_NEW)) {
        //新订单
        transactionType = "6";
      }
    }
    ...
    if ("6".equals(transactionType)) {
      ...
      logger.debug("creating transaction");
      TransactionResponse transactionResponse = client
          .createTransaction(parentOrderNum, transactionRequest);
      if (!transactionResponse.getRespcode()
          .equals(Constant.SERVICE_B2C_PAYMENT_RESPONSE_CODE_SUCCESS)) {
...
//        logger.info("orderIds=================" + orderIds);
    ...
    logger.debug("totalFee=" + ESAPI.encoder()
        .encodeForHTML(String.valueOf(queryOrderByParentOrderNumResponse.getPrice())));
    wechatPrepayResponse = demo.getPackage(tpWxPay);
