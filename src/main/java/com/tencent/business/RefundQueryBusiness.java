...
public class RefundQueryBusiness {
    ...
    public void run(RefundQueryReqData refundQueryReqData,RefundQueryBusiness.ResultListener resultListener) throws Exception {
        ...
        log.i(ESAPI.encoder().encodeForHTML(refundQueryServiceResponseString));
        ...
        if(Configure.isUseThreadToDoReport()){
            ...
            Util.log("pay+report总耗时（异步方式上报）：" + ESAPI.encoder().encodeForHTML((timeAfterReport - costTimeStart)+"") + "ms");
        }else{
            ReportService.request(reportReqData);
            timeAfterReport = System.currentTimeMillis();
            Util.log("pay+report总耗时（同步方式上报）：" + ESAPI.encoder().encodeForHTML((timeAfterReport - costTimeStart)+"") + "ms");
        }
        ...
        if (refundQueryResData.getReturn_code().equals("FAIL")) {
            ...
            if (refundQueryResData.getResult_code().equals("FAIL")) {
                Util.log("出错，错误码：" + ESAPI.encoder().encodeForHTML(refundQueryResData.getErr_code()) + "     错误信息：" + ESAPI.encoder().encodeForHTML(refundQueryResData.getErr_code_des()));
    ...
    private void getRefundOrderListResult(String refundQueryResponseString) throws ParserConfigurationException, SAXException, IOException {
        ...
        for(RefundOrderData refundOrderData : refundOrderList){
            Util.log("退款订单数据NO" + ESAPI.encoder().encodeForHTML(count+"") + ":");
            Util.log(ESAPI.encoder().encodeForHTML(refundOrderData.toMap()));
            orderListResult += refundOrderData.toMap().toString();
            count++;
        }
        log.i("查询到的结果如下：");
        log.i(ESAPI.encoder().encodeForHTML(orderListResult));
    }
    ...
    public void setResult(String result,String type){
        setResult(result);
        log.log(ESAPI.encoder().encodeForHTML(type),ESAPI.encoder().encodeForHTML(result));
    }
