...
public class ScanPayBusiness {
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        log.i("api请求总耗时：" + ESAPI.encoder().encodeForHTML(totalTimeCost+"") + "ms");

        //打印回包数据
        log.i(ESAPI.encoder().encodeForHTML(payServiceResponseString));
        ...
        if (Configure.isUseThreadToDoReport()) {
            ...
            log.i("pay+report总耗时（异步方式上报）：" + ESAPI.encoder().encodeForHTML((timeAfterReport - costTimeStart)+"") + "ms");
        } else {
            ReportService.request(reportReqData);
            timeAfterReport = System.currentTimeMillis();
            log.i("pay+report总耗时（同步方式上报）：" + ESAPI.encoder().encodeForHTML((timeAfterReport - costTimeStart)+"") + "ms");
        }
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                //出现业务错误
                log.i("业务返回失败");
                log.i("err_code:" + ESAPI.encoder().encodeForHTML(errorCode));
                log.i("err_code_des:" + ESAPI.encoder().encodeForHTML(errorCodeDes));
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    if (errorCode.equals("AUTHCODEEXPIRE")) {
                        ...
                        log.w("【支付扣款明确失败】原因是：" + ESAPI.encoder().encodeForHTML(errorCodeDes));
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        log.i(ESAPI.encoder().encodeForHTML(payQueryServiceResponseString));
        ...
        if (scanPayQueryResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            log.i("支付订单查询API系统返回失败，失败信息为：" + ESAPI.encoder().encodeForHTML(scanPayQueryResData.getReturn_msg()));
            return false;
        } else {
            if (scanPayQueryResData.getResult_code().equals("SUCCESS")) {//业务层成功
                ...
                log.i("查询出错，错误码：" + ESAPI.encoder().encodeForHTML(scanPayQueryResData.getErr_code()) + "     错误信息：" + ESAPI.encoder().encodeForHTML(scanPayQueryResData.getErr_code_des()));
                return false;
            }
        }
    }
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
        ...
        log.i("撤销API返回的数据如下：");
        log.i(ESAPI.encoder().encodeForHTML(reverseResponseString));
        //将从API返回的XML数据映射到Java对象
        ...
        if (reverseResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            log.i("支付订单撤销API系统返回失败，失败信息为：" + ESAPI.encoder().encodeForHTML(reverseResData.getReturn_msg()));
            return false;
        } else {
            if (reverseResData.getResult_code().equals("FAIL")) {
                log.i("撤销出错，错误码：" + ESAPI.encoder().encodeForHTML(reverseResData.getErr_code()) + "     错误信息：" + ESAPI.encoder().encodeForHTML(reverseResData.getErr_code_des()));
